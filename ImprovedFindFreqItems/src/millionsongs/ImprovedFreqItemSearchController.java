...
public class ImprovedFreqItemSearchController {
...
//		countAllItemsFreq(localArgs);
		...
		while(true){
			...
			if(k==2){
				...
				//Generate Candidates
				candidates = candidateGenerator.generateCandidates(location+"FreqItems"+(k-1)+"/itemsets",conf);
			}
			...
			if(k > 2)
				transactionLoc = location+"FreqItems"+(k-1)+"/transactions";


			//Generate K frequent Itemset
			conf.set("K", String.valueOf(k));
			long itemsetNumber = KFreqItemsetTaskImproved.run(conf, k, transactionLoc, location,args[2]);
			if(itemsetNumber == -1){
