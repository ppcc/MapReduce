...
public class ImprovedAprori {
	 ...
	 * Meanwhile, generating a map in which key is item id and
	 * value is a list of transactionId that contains this item id.
	 * One line, one transaction
	 * @param filename
	 * @param oneFreqItems	itemId and its counts
	 * @param transMap itemId and the list of transaction id that contains this item
	 * @return
	 * @throws IOException
	 */
	public List<List<Integer>> loadTransactions(String filename, Map<Integer,Integer> oneFreqItems,Map<Integer,List<Integer>> transMap) throws IOException{
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (line.trim().length() > 0) {
					...
					//Omit the first item(assuming that first item is user id)
	 ...
	 *
	 */
	public void generateAllFreqItemsets(String transactionFile, String outputFolder) throws IOException {
		...
		//Prepare
		List<List<Integer>> transactions =  loadTransactions(transactionFile,oneItemFreq,itemTransMap);
		...
		while(true){
			...
			for(List<Integer> candidate : candidates) {
				System.out.format("Visiting candidates:%d/%d\r", t, candidates.size());
				count = 0;
	 ...
	 * Write L_k into file
	 * one line, one set
	 * @param Lk k frequent item set
	 * @param k k
	 * @throws IOException
	 */
	private void writeToFile(List<List<Integer>> Lk, int k, String folder) throws IOException {
